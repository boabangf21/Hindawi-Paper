function [ cutobaseChannelGainMatrix, d2dttobaseChannelGainMatrix, cutod2drChannelGainMatrix, d2dttod2drChannelGainMatrix] = channelGen(noCUusers, noD2Dusers, nochannels,transReciDisMin,transReciDisMax )
...
%noCUusers = 2;
%noD2Dusers = 3;
cellRadiusMax = 400; cellRadiusMin = 100;
...
disMobile2Base = cellRadiusMin + (cellRadiusMax - cellRadiusMin)*rand(1,4000);
tempphi = rand(1,4000)*2*pi;
...
mobileDistancePair = abs(mobileCoordinateRow(ones(4000,1),:)-mobileCoordinateColumn(:,ones(1,4000)));
...
for i = 2:noD2Dusers
    ...
    if i == 2
        tempd2dCandidateIndex = find((tempd2dtod2dDisPair >= 100)&(tempd2dtod2dDisPair <= 800));
    else
        tempd2dCandidateIndex = find(sum((tempd2dtod2dDisPair >= 100)&(tempd2dtod2dDisPair <= 800)) == i-1);
    end
...
cutobaseChannelGainMatrix = db2lin(-33 - 33.*log10(cutobaseDistanceArray(ones(nochannels,1),:)) - shadowing1 + 65);
...
%    shadowing2(i,:) = mean(B);
%end  
d2dttobaseChannelGainMatrix = db2lin(-33 - 33.*log10(d2dttobaseDistanceArray(ones(nochannels,1),:)) - shadowing2 + 65);
...
%        C = normrnd(0,4,50, noCUusers);
 ...
 %   end
%end
cutod2drChannelGainMatrix = db2lin(-33 - 33.*repmat(log10(cutod2drDistanceMatrix),[1 1 nochannels]) - shadowing3 + 65);
...
%    end
%end
d2dttod2drChannelGainMatrix = db2lin(-33 - 33.*repmat(log10(d2dtod2drDistanceMatrix),[1 1 nochannels]) - shadowing4 + 58);
